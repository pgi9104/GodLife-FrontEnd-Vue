<template>
  <div class="container">
	...
	<div class="row">
		<div class="col-10">
			<label for="projectCodeSel">프로젝트 코드</label>
			<select id="projectCodeSel" name="projectCodeSel">
			</select>
		</div>
		<div class="col-2">
  ...
  <div id="scriptModalId" class="modal">
	  <div class="modal-content animate" style="height: 80vh;">
		...
		<div>
			<select id="dbType" style="width: 20vw;" :onchange="script">
		...
		<textarea id="scriptResult" style="height: 100vh;"></textarea>
	</div>
  </div>
</template>
...
import { ALIGN, DataType, GRID_MSG, HeaderInfo, HeaderInfoBuilder, OptionInfo, TableBuilder } from '@/utils/tableBuilder';
...
const getApi = () =>{
	...
	http.setSuccess(function(res: any){
		...
		initdata.list = res.data.list;
		initdata.save = res.data.save;
		initdata.script = res.data.script;
	});
...
const get = () =>{
	...
	let http = new Http();
	http.setParam(null);
	http.setUrl(initdata.list.href+"?projectCode="+(getId('projectCodeSel') as HTMLSelectElement).value);
	...
	http.setSuccess(function(res: any){
		const data = res.data;
		console.log(data);
		const selBox = (getId('projectCodeSel') as HTMLSelectElement);
		selBox.innerHTML='';
		data.selectBox.forEach((option : string) => {
			const opt = new OptionInfo();
			opt.text = option;
			opt.value = option;
			selBox.appendChild(opt.createOption());
		});
...
const buildGrid = () => {
	...
	const headerInfo =[
		['no','','상태','프로젝트코드','테이블명','테이블컬럼','컬럼 타입','컬럼 타입 제한','필수여부', 'PK 여부', '기본값', '설명', '정렬 순번']
	];
	
	let headers: HeaderInfo[] = [
		{colId: 'no', width: '50px', dtType: DataType.READ_ONLY, align: ALIGN.CENTER},
		{colId: 'chkbox', width: '20px', dtType: DataType.CHECK, align: ALIGN.CENTER},
		{colId: 'status', width: '50px', dtType: DataType.READ_ONLY, align: ALIGN.CENTER},
		{colId: 'projectCode', width: '80px', dtType: DataType.TEXT, align: ALIGN.LEFT},
		{colId: 'tableName', width: '80px', dtType: DataType.TEXT, align: ALIGN.LEFT},
		{colId: 'tableCol', width: '80px', dtType: DataType.TEXT, align: ALIGN.LEFT},
		{colId: 'colType', width: '80px', dtType: DataType.SELECT, align: ALIGN.LEFT},
		{colId: 'colTypeLmt', width: '100px', dtType: DataType.TEXT, align: ALIGN.RIGHT},
		{colId: 'nullable', width: '80px', dtType: DataType.SELECT, align: ALIGN.LEFT},
		{colId: 'colPk', width: '80px', dtType: DataType.SELECT, align: ALIGN.LEFT},
		{colId: 'defaultValue', width: '80px', dtType: DataType.TEXT, align: ALIGN.LEFT},
		{colId: 'comment', width: '80px', dtType: DataType.TEXT, align: ALIGN.LEFT},
		{colId: 'sortSeq', width: '80px', dtType: DataType.TEXT, align: ALIGN.RIGHT},
	].map((item, colIdx)=>{
